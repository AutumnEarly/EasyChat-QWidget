...
BubbleFrame::BubbleFrame(ChatMsgRole role, QWidget *parent)
    :QFrame(parent)
    ,m_role(role)
    ,m_margin(10)
{
...
ChatMsgRole BubbleFrame::role() const
{
    return m_role;
}

void BubbleFrame::setRole(ChatMsgRole newRole)
{
    if (m_role == newRole)
        return;
...
QColor BubbleFrame::backColor() const
{
    return m_backColor;
}

void BubbleFrame::setBackColor(const QColor &newBackColor)
{
